...
$(document).on("click", ".suspend-alert", function(e) {
    e.preventDefault();
    var idAtr = $(this).closest(".suspend-alert").attr("id");
    var id = idAtr.split(".");
    bootbox.confirm({
        ...
        callback: function (result) {
            if(result === true){
                var url = "http://ussd.ultramedhealth.com/api/v1/dashboard/admin/member/update/suspend/" + id[1];
                fetch(url, {
                    method: 'POST',
                    cache: 'no-cache',
                    headers: {
